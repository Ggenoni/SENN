...
def show_explanations(
    model, test_loader, dataset, num_explanations=2, save_path=None, concept_names=None,
    ...
    if selected_indices is not None:
        if not all(0 <= idx < len(full_dataset) for idx in selected_indices):
            raise ValueError(f"All selected indices must be within the range [0, {len(full_dataset) - 1}].")
        batch_idx = selected_indices
    else:
        batch_idx = np.random.randint(0, len(full_dataset), num_explanations)

    # Process each selected index
    for i, idx in enumerate(batch_idx):
        # Retrieve sample and label directly from the dataset
        sample_input, sample_label = full_dataset[idx]
        sample_input = sample_input.unsqueeze(0).to(device)

        # Feed sample to model to obtain explanation
        with torch.no_grad():
            y_pred, (concepts, relevances), _ = model(sample_input)
        ...
        if concept_names is not None:
            ...
            create_barplot(ax1, relevances[0], y_pred[0], x_label='Relevances (theta)',
                           concept_names=concept_names, **kwargs)
            ax1.xaxis.set_label_position('top')
            ax1.tick_params(which='major', labelsize=12)

            create_barplot(ax2, concepts[0], y_pred[0], x_lim=concept_lim,
            ...
            ax1.imshow(sample_input.squeeze().cpu(), cmap='gray')
            ax1.set_axis_off()
            ax1.set_title(f'Input Prediction: {y_pred[0].item()}', fontsize=18)

            create_barplot(ax2, relevances[0], y_pred[0], x_label='Relevances (theta)', **kwargs)
            ax2.xaxis.set_label_position('top')
            ax2.tick_params(which='major', labelsize=12)

            create_barplot(ax3, concepts[0], y_pred[0], x_lim=concept_lim,
