...
class LimeWrapper:
    ...
    def __call__(self, inputs):
        """
        Makes the wrapper callable, so it can directly process inputs.
        - inputs: A NumPy array from LIME with shape (batch_size, height, width, channels)
        """
        # Convert NumPy array to PyTorch tensor
        inputs = torch.tensor(inputs, dtype=torch.float32).permute(0, 3, 1, 2)  # HWC -> NCHW

        # Ensure compatibility with GPU
        if torch.cuda.is_available():
            inputs = inputs.cuda()

        # Pass inputs through the model
        with torch.no_grad():
            predictions, _, _ = self.model(inputs)  # Extract predictions only
            probabilities = F.softmax(predictions, dim=1)  # Convert logits to probabilities

        # Convert back to NumPy for LIME
...
class LimeWrapper:
    def __init__(self, model):
        self.model = model
        self.resize = Resize((28, 28))  # Ensure all inputs are resized to 28x28

    def __call__(self, inputs):
        """
        Handles inputs from LIME, resizes them, and ensures proper shape for the model.
        - inputs: A NumPy array from LIME with shape (batch_size, height, width, channels).
        """
        # Convert NumPy array to PyTorch tensor
        inputs = torch.tensor(inputs, dtype=torch.float32)

        # Check input dimensions and reshape properly
        if inputs.ndimension() == 4:  # Input has 4 dimensions (batch_size, height, width, channels)
            if inputs.shape[-1] == 3:  # Convert RGB to grayscale
                inputs = inputs.mean(dim=-1, keepdim=True)  # Average over color channels
            inputs = inputs.permute(0, 3, 1, 2)  # Convert HWC -> NCHW
        elif inputs.ndimension() == 3:  # Input has 3 dimensions (batch_size, height, width)
            inputs = inputs.unsqueeze(1)  # Add a channel dimension (N, 1, H, W)

        # Resize inputs to ensure compatibility with the model
        inputs = F.interpolate(inputs, size=(28, 28), mode='bilinear', align_corners=False)

        # Ensure the shape is [batch_size, 1, 28, 28]
        if inputs.shape[1:] != (1, 28, 28):  # Expected MNIST shape
            raise RuntimeError(
                f"Input shape mismatch. Expected [batch_size, 1, 28, 28], got {inputs.shape}."
            )
